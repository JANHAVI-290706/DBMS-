...
-- 'root@localhost', 'RVU-PC-007', '8.4.6', '2025-09-15 06:06:40'
...
SELECT Name, Salary, Salary * 0.10 AS Bonus, Salary + (Salary * 0.10) AS Total_Salary_With_Bonus FROM Employees;
...
SELECT Name, Salary, Salary * 0.15 AS Tax, Salary - (Salary * 0.15) AS Salary_After_Tax FROM Employees;
...
SELECT Name, Salary AS Monthly_Salary, Salary * 12 AS Yearly_Salary FROM Employees;
...
SELECT Name FROM Employees WHERE YEAR(HireDate) = 2023 AND Name NOT IN (SELECT Name FROM Employees WHERE YEAR(HireDate) = 2022);
/*'Ivy Green'*/
