...
SELECT SUBSTRING(Title, 1, 3) AS FirstThreeLetters FROM Books;
/*'The'
'Ato'
'Cle'
'Thi'
...
SELECT CONCAT(Author, ' - ', Title) AS AuthorAndTitle FROM Books;
...
SELECT Name, DATEDIFF(CURDATE(), JoinDate) AS DaysSinceJoined FROM Customers;
/*'Arjun Rao', '1680'
'Priya Nair', '1880'
'John Smith', '1342'
'Maria Lopez', '2118'*/
...
SELECT C.Name, SUM(O.Quantity) AS TotalBooksOrdered FROM Orders O JOIN Customers C ON O.CustID = C.CustID GROUP BY C.Name;
/*'Arjun Rao', '3'
'Priya Nair', '1'
'John Smith', '3'
'Maria Lopez', '1'*/

SELECT B.Title, AVG(O.Quantity) AS AvgOrderQuantity FROM Orders O JOIN Books B ON O.BookID = B.BookID GROUP BY B.Title;
...
SELECT B.Title, SUM(O.Quantity) AS TotalQuantityOrdered FROM Orders O JOIN Books B ON O.BookID = B.BookID GROUP BY B.Title ORDER BY TotalQuantityOrdered DESC LIMIT 1;
/*'Python Crash Course', '3'*/

SELECT B.Title, SUM(B.Price * O.Quantity) AS TotalRevenue FROM Orders O JOIN Books B ON O.BookID = B.BookID GROUP BY B.Title;
...
SELECT EXTRACT(YEAR FROM OrderDate) AS OrderYear, COUNT(*) AS TotalOrders FROM Orders GROUP BY EXTRACT(YEAR FROM OrderDate) ORDER BY OrderYear;
/*'2020', '1'
'2021', '2'
'2022', '2'*/
